<?php
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST'){
                ...
                }elseif (!is_numeric($student_id) || strlen($student_id) !== 12) {
                        $student_idErr = 'Student ID must be exactly 12 digits long';
                }
                if(empty($contact)){
                        $contactErr = 'Contact is Required';
                }elseif (!preg_match('/^\d{11}$/', $contact)) {
                        $contactErr = 'Contact must be exactly 11 digits long';
                    }
                if (empty($grade_section)) {
                        $grade_sectionErr = 'Grade & Section is Required';
                } elseif (!preg_match('/^(1[0-2]|[7-9]) [A-Z]+$/', $grade_section)) {
                        $grade_sectionErr = 'Grade & Section must start with a number from 7 to 12 followed by a space and all uppercase letters (e.g., 10 CONFUCIUS)';
                }
                if(empty($name)){
                ...
                if(empty($borrow_date)){
                        $borrow_dateErr = 'Quantity is Required';
                } elseif ($borrow_date < date('Y-m-d')) {
                        $borrow_dateErr = 'Borrow date cannot be before today';
                    }
                if(empty($expected_return_date)){
                        $expected_return_dateErr = 'Quantity is Required';
                } elseif ($expected_return_date < $borrow_date) {
                        $expected_return_dateErr = 'Expected return date cannot be before borrow date';
                    }
                
                if(empty($firstnameErr) && empty($lastnameErr) && empty($contactErr) && empty($student_idErr) && empty($grade_sectionErr) && empty($borrow_dateErr) &&empty($expected_return_dateErr)){
                        ...
                        if($productObj->add()){
                                ...
                                echo ' Cant add laboratory apparatuses, something went wrong';
                        }
                }
        }
                                                ...
                                                <div class="table1">
                                                        <div class="info">
                                                                <div class="">             
                                                                        ...
                                                                        <br>                                              
                                                                        <label for="first_name" value="<?=$first_name?>">First Name</label><span class="error"></span> 
